    ...
    with tab_manual:
        ...
        if st.button("➕ ثبت کالا", type="primary", use_container_width=True):
            if not pn: st.error("نام الزامی است.")
                ...
                # اصلاح: اضافه کردن میکروثانیه تا در ثبت سریع پشت سر هم، کد تکراری نشود
                fc = p_code.strip() if p_code.strip() else "AUTO-"+get_iran_time().strftime("%Y%m%d%H%M%S%f")
                try:
                    conn = sqlite3.connect(DB_NAME)
                    c = conn.cursor()
...
# ==========================================
    ...
    if not sales_df.empty:
        # اصلاح: برخی نسخه‌های pandas روی datetime دارای timezone از tz_localize ارور می‌دن
        # بنابراین ابتدا بررسی می‌کنیم و در صورت وجود timezone از tz_convert استفاده می‌کنیم
        _ts = pd.to_datetime(sales_df['timestamp'])
        sales_df['timestamp'] = _ts.dt.tz_convert(None) if _ts.dt.tz is not None else _ts
        # اصلاح: ستون‌های عددی ممکن است در رکوردهای قدیمی NaN داشته باشند
...
# ==========================================
    ...
    def render_ledger(l_type, title, p_label):
        ...
        if not df.empty:
            ...
            did = st.number_input(f"برای حذف {title} کد را وارد کنید:", min_value=1, key=f"d_{l_type}")
